...
def get_inventory_inputs():
    ...
    for rad, widget_tuple in rf2_widgets.items():
        ...
        rf2_value = text_widget.get("1.0", "end-1c").strip()
        rf2[rad] = float(rf2_value) if rf2_value and rf2_value.lower() != "none" else None  # ✅ Fix here

        #rf2[rad] = float(value) if value and value.lower() != "none" else None
    ...
    for rad, widget_tuple in rf3_widgets.items():
        text_widget = widget_tuple[1]
        rf3_value = text_widget.get("1.0", "end-1c").strip()
        rf3[rad] = float(rf3_value) if rf3_value and rf3_value.lower() != "none" else None  # ✅ Fix here

        #rf3[rad] = float(value) if value and value.lower() != "none" else None
...
def apply_config(config_data):
    """Apply configuration settings to the GUI fields."""
    try:
        output_filename_entry.delete(0, tk.END)
        output_filename_entry.insert(0, config_data.get('output_filename', 'hazcat_out.txt'))

        selected_list_box.delete(0, tk.END)
        for rad in config_data.get('rads_list', []):
            selected_list_box.insert(tk.END, rad)

        generate_input_rows()

        for rad, inv in zip(config_data.get('rads_list', []), config_data.get('inventories', [])):
            if rad in inventory_widgets:
                inventory_widgets[rad][1].delete("1.0", tk.END)
                inventory_widgets[rad][1].insert("1.0", str(inv))

        if config_data.get('Rs_HC2') is not None:
            for rad, rf2 in zip(config_data.get('rads_list', []), config_data['Rs_HC2']):
                if rad in rf2_widgets:
                    rf2_widgets[rad][1].delete("1.0", tk.END)
                    rf2_widgets[rad][1].insert("1.0", str(rf2))

        if config_data.get('Rs_HC3') is not None:
            for rad, rf3 in zip(config_data.get('rads_list', []), config_data['Rs_HC3']):
                if rad in rf3_widgets:
                    rf3_widgets[rad][1].delete("1.0", tk.END)
                    rf3_widgets[rad][1].insert("1.0", str(rf3))
    except Exception as e:
...
# Add a button to load config file
load_config_button = ttk.Button(window, text="Load Config File", command=load_config_file)
load_config_button.grid(row=0, column=10, padx=1, pady=1)
